...
def index(request):
    ...
    cameras_empty = cameras_total.filter(status="empty").exclude(multiple_backs=True)
...
def patterns(request):
    ...
    roll1 = {"film": {"type": "e6", "iso": 400}}
    roll2 = {
        "push_pull": "+2",
        "get_push_pull_display": "Push 2 stops",
        "effective_iso": 1600,
        "film": {"type": "c41", "iso": 400},
    }
    roll3 = {
        "push_pull": "-1",
...
def account_settings(request):
    if request.method == "POST":
            ...
            for field in user_form:
                for error in field.errors:
                    messages.add_message(
                        request, messages.ERROR, f"{field.name.capitalize()}: {error}"
                    )
            return redirect(reverse("settings"))
    else:
        ...
        exportable = {
            ...
            "camera-backs": CameraBack.objects.filter(
...
def stocks(request, manufacturer="all"):
    ...
    if request.GET.get("manufacturer") and request.GET.get("manufacturer") != "all":
        ...
        if not request.htmx:
            return redirect(
                reverse("stocks-manufacturer", args=(filters["manufacturer"],))
                + type_passthrough
            )
...
def stock(request, manufacturer, slug):
    ...
    if request.user.is_authenticated:
        ...
        films = (
            Film.objects.filter(stock=stock)
            .exclude(Q(personal=True) & ~Q(added_by=request.user))
    ...
    for film in films:
        ...
        if request.user.is_authenticated:
            ...
            user_history_count = (
                Roll.objects.filter(owner=request.user, film=film)
    ...
    total_rolls_table = {
        "headings": headings,
        "rows": [{"columns": [film["name"], film["count"]]} for film in films_list],
    }
    total_inventory_table = {
        "headings": headings,
        "rows": [
            {"columns": [film["name"], film["user_inventory_count"]]}
        ...
        "rows": [
            {
                "columns": [
                    {
                        "title": film["name"],
                        "href": f'{film["url"]}#film_history_heading',
                    },
...
def inventory(request):
    ...
    format_counts = (
        Film.objects.filter(
            roll__owner=request.user, roll__status=status_number("storage")
        )
        .values("format")
        .distinct()
        .order_by("-format")
    ...
    for format in format_counts:
        format["format_display"] = force_str(
            format_choices[format["format"]], strings_only=True
        )
    ...
    for type in type_counts:
        if type["stock__type"] is not None:
            type["type_display"] = force_str(
                type_choices[type["stock__type"]], strings_only=True
            )
...
def logbook(request):
    ...
    rolls = (
        Roll.objects.filter(owner=owner)
        .exclude(status=status_number("storage"))
        .order_by("status", "-ended_on", "-started_on", "-code")
    )
    year = ""
    all_years = {}
...
def ready(request):
    ...
    rolls = Roll.objects.filter(
        owner=request.user, status=status_number("shot")
    ).order_by("-ended_on", "-started_on", "-code")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if project.status == "archived":
                rolls = Roll.objects.filter(
                    owner=request.user, project=project, status=status_number("storage")
                )
...
def project_delete(request, pk):
    ...
    if roll_count:
        messages.success(
            request,
            f"Project deleted and {roll_count} {plural} now available for other projects.",
        )
    else:
...
def project_detail(request, pk):
    ...
    film_counts = total_film_count.annotate(count=Count("roll")).order_by(
        "stock__type",
        "-format",
    ...
    roll_logbook = (
        Roll.objects.filter(owner=owner, project=project)
        .exclude(status=status_number("storage"))
        .order_by("status", "-ended_on", "-started_on", "-code")
    )
    ...
    if request.htmx:
        response = render(
            request,
            "partials/project-camera-logbook-wrapper.html",
            {
                "project": project,
                "cameras": cameras,
                "roll_logbook": roll_logbook,
        ...
        response["HX-Push"] = (
            reverse("project-detail", args=(project.id,)) + querystring
        )
...
def project_rolls_remove(request, pk):
    ...
    if roll_count:
        plural = pluralize("roll", roll_count)
        messages.success(
            request, f"Removed {roll_count} {plural} of {film} from this project!"
        )
    else:
...
def film_rolls(request, stock=None, format=None, slug=None):
    ...
    rolls_history = rolls.exclude(status=status_number("storage")).order_by(
        "-started_on"
    )
        ...
        if form.is_valid():
            if form.cleaned_data["new_manufacturer"]:
                ...
                try:
                    ...
                    messages.error(
                        request, "There’s already a manufacturer with that name."
                    )
                    context = {
                        "form": form,
                        "js_needed": True,
            ...
            messages.success(
                request, f"New film stock “{stock}” (in {format_message}) added!"
            )
...
def roll_delete(request, pk):
    ...
    messages.success(request, "Roll of %s successfully deleted." % (film))
    return redirect(reverse("index"))
...
def roll_journal_add(request, roll_pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                Journal.objects.create(roll=roll, date=date, notes=notes, frame=frame)
        ...
        try:
            starting_frame = Journal.objects.filter(roll=roll).reverse()[0].frame + 1
        except IndexError:
            starting_frame = 1
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                return redirect(
                    reverse("roll-journal-detail", args=(roll.id, entry.id))
                )
            except IntegrityError:
                messages.error(request, "Only one entry per date per roll.")
                return redirect(reverse("roll-journal-edit", args=(roll.id, entry.id)))
        else:
            messages.error(request, "Something is not right.")
            return redirect(reverse("roll-journal-edit", args=(roll.id, entry.id)))
    else:
...
def roll_journal_delete(request, roll_pk, entry_pk):
    ...
    messages.success(
        request, "Journal entry for %s successfully deleted." % (entry_date)
    )
    return redirect(reverse("roll-detail", args=(roll.id,)))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                if "another" in request.POST:
                    return redirect(
                        reverse("roll-frame-add", args=(frame.roll.id,)) + "?another"
                    )
                else:
                    return redirect(reverse("roll-detail", args=(roll.id,)))
            except IntegrityError:
        ...
        try:
            previous_frame = (
                Frame.objects.filter(roll=roll).order_by("number").reverse()[0]
            )
        ...
        enhanced_label_aperture = {"before": "ƒ/"}
...
def roll_frame_detail(request, roll_pk, number):
    frame = get_object_or_404(
        Frame, roll__id=roll_pk, roll__owner=request.user, number=number
    )
    previous_frame = Frame.objects.filter(
        number=number - 1, roll__id=roll_pk, roll__owner=request.user
    ).first()
...
def roll_frame_edit(request, roll_pk, number):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Custom fields take precedence over dropdown presets.
            if (
                form.cleaned_data["aperture"]
                and form.cleaned_data["aperture_preset"] == ""
            ):
                frame.aperture = form.cleaned_data["aperture"]
            elif form.cleaned_data["aperture_preset"] != "":
                frame.aperture = form.cleaned_data["aperture_preset"]
            ...
            if (
                form.cleaned_data["shutter_speed"]
                and form.cleaned_data["shutter_speed_preset"] == ""
            ):
                frame.shutter_speed = form.cleaned_data["shutter_speed"]
            elif form.cleaned_data["shutter_speed_preset"] != "":
                frame.shutter_speed = form.cleaned_data["shutter_speed_preset"]
...
def camera_or_back_load(request, pk, back_pk=None):
    ...
    if request.method == "POST":
        ...
        if request.POST.get("project"):
            current_project = get_project_or_none(
                Project, owner, request.POST.get("project")
            )

        # The the oldest roll we have of that film in storage.
        if current_project is not None and current_project != 0:
            roll = rolls.filter(project=current_project).order_by("created_at")[0]
        else:
        ...
        roll.save()
        messages.success(
            request, f"{camera_or_back} loaded with {roll.film} (code: {roll.code})!"
        )

        if current_project:
            return redirect(reverse("project-detail", args=(current_project.id,)))
        else:
            return redirect(reverse("roll-detail", args=(roll.id,)))
    else:
        ...
        if current_project is not None and current_project != 0:
            ...
            film_counts = (
                Film.objects.filter(
                    roll__owner=owner, roll__status=status_number("storage")
                )
                .annotate(count=Count("format"))
                .order_by(
                    "stock__type",
...
def camera_or_back_detail(request, pk, back_pk=None):
    ...
    if back_pk:
        camera_back = get_object_or_404(
            CameraBack, id=back_pk, camera__owner=request.user
        )
        camera_or_back = camera_back
    else:
        camera_back = None
        ...
        if camera_back:
            if camera_back.status == "loaded":
                roll = Roll.objects.filter(
                    camera_back=camera_back, status=status_number("loaded")
                )[0]
        else:
            if camera.status == "loaded":
                roll = Roll.objects.filter(
                    camera=camera, status=status_number("loaded")
                )[0]
...
def camera_add(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if camera.multiple_backs:
                success_message += (
                    " To be able to load this camera, add one or more camera backs."
                )
...
def camera_back_add(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, "Camera back for %s added!" % (camera.name))
...
def camera_delete(request, pk):
    ...
    messages.success(request, f"Camera “{name}” successfully deleted.")
    return redirect(reverse("index"))
...
def camera_back_delete(request, pk, back_pk):
    ...
    messages.success(request, "%s successfully deleted." % (name))
    return redirect(reverse("camera-detail", args=(camera.id,)))
    ...
    def post(self, request, *args, **kwargs):
        ...
        for row in reader:
            ...
            if created:
                ...
                if row["camera_id"]:
                    obj.camera = get_object_or_404(
                        Camera, id=row["camera_id"], owner=request.user
                    )
                if row["camera_back_id"]:
                    obj.camera_back = get_object_or_404(
                        CameraBack, id=row["camera_back_id"], camera__owner=request.user
                    )
                if row["project_id"]:
                    obj.project = get_object_or_404(
                        Project, id=row["project_id"], owner=request.user
                    )

                # Add optional dates
                if row["started"]:
                    obj.started_on = datetime.datetime.strptime(
                        row["started"], "%Y-%m-%d"
                    ).date()
                if row["ended"]:
                    obj.ended_on = datetime.datetime.strptime(
                        row["ended"], "%Y-%m-%d"
                    ).date()
    ...
    def post(self, request, *args, **kwargs):
        ...
        for row in reader:
            ...
            if created:
                ...
                CameraBack.objects.filter(
                    id=row["id"], camera__owner=request.user
                ).update(
                    notes=row["notes"],
                    status=row["status"],
                    # Keep the original created and updated dates and times.
    ...
    def get(self, request, *args, **kwargs):
        ...
        for project in projects:
            ...
            for roll in roll_objects:
                ...
                roll_name = (
                    f"{roll_code}{roll.film.__str__()} / {roll.get_status_display()}"
                )
            ...
            if created:
                ...
                if camera_ids:
                    for id in camera_ids:
                        obj.cameras.add(
                            get_object_or_404(Camera, id=id, owner=request.user)
                        )
