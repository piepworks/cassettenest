...
def htmx_request(
    user=None, get_url=None, post_url=None, post_data={}, trigger=None, **kwargs
):
...
class InventoryTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        baker.make(
            ...
            owner=cls.user,
            film=baker.make(
                Film, slug="slug", stock=baker.make(Stock, name="Stock Name")
            ),
        )
        baker.make(
            Roll,
            owner=cls.user,
            film=baker.make(Film, name="No Stock", slug="no-stock"),
        )
...
class LogbookTests(TestCase):
    ...
    def test_status_logbook_page(self):
        ...
        self.assertEqual(
            response.context["bulk_status_next"], bulk_status_next_keys[status]
        )
...
class ImportTests(TestCase):
    ...
    def test_import_rolls_success(self):
        ...
        self.assertEqual(
            response1.get("Content-Disposition"), 'attachment; filename="rolls.csv"'
        )
    ...
    def test_import_cameras_success(self):
        ...
        self.assertEqual(rows, 1)
        self.assertEqual(
            response1.get("Content-Disposition"), 'attachment; filename="cameras.csv"'
        )
    ...
    def test_import_projects_success(self):
        ...
        self.assertEqual(rows, 1)
        self.assertEqual(
            response1.get("Content-Disposition"), 'attachment; filename="projects.csv"'
        )
    ...
    def test_import_journals_success(self):
        ...
        self.assertEqual(rows, 1)
        self.assertEqual(
            response1.get("Content-Disposition"), 'attachment; filename="journals.csv"'
        )
    ...
    def test_import_frames_success(self):
        ...
        self.assertEqual(rows, 1)
        self.assertEqual(
            response1.get("Content-Disposition"), 'attachment; filename="frames.csv"'
        )
...
class JournalTests(TestCase):
    ...
    def test_journal_edit_post(self):
        ...
        self.assertRedirects(
            response1,
            reverse("roll-journal-detail", args=(self.roll.id, self.entry.id)),
        )
...
class FilmRollsTests(TestCase):
    ...
    def test_personal_film_rolls_404_with_stock(self):
        film = baker.make(
            Film, stock=baker.make(Stock), personal=True, added_by=baker.make(User)
        )
        response = self.client.get(
            reverse(
                "film-rolls",
    ...
    def test_film_rolls_without_stock(self):
        response = self.client.get(
            reverse("film-slug-redirect", args=(self.film.slug,))
        )
        self.assertEqual(response.status_code, 302)

    def test_personal_film_rolls_404_without_stock(self):
        film = baker.make(
            Film, slug="private-slug", personal=True, added_by=baker.make(User)
        )
...
class ProjectTests(TestCase):
    ...
    def test_project_edit_post(self):
        ...
        project = Project.objects.get(owner=self.user, name="A Project")
        response = self.client.post(
            reverse("project-edit", args=(project.id,)),
            data={
                "name": "A New Name",
                "status": "current",
            },
    ...
    def test_project_edit_post_archived_with_rolls(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn(
            "Project archived and 1 roll now available for other projects.", messages
        )
    ...
    def test_project_delete_with_rolls(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn(
            "Project deleted and 2 rolls now available for other projects.", messages
        )
    ...
    def test_project_detail_htmx(self):
        ...
        headers = {"HTTP_HX-Request": "true"}
        response = self.client.get(
            reverse("project-detail", args=(project.id,)), **headers
        )

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(
            response, "partials/project-camera-logbook-wrapper.html"
        )
    ...
    def test_project_rolls_add(self):
        ...
        response = self.client.post(
            reverse("project-rolls-add", args=(project.id,)),
            data={"quantity": 1, "film": film.id},
        )
        messages = [m.message for m in get_messages(response.wsgi_request)]
    ...
    def test_project_rolls_remove(self):
        ...
        response = self.client.post(
            reverse("project-rolls-remove", args=(project.id,)), data={"film": film.id}
        )
        messages = [m.message for m in get_messages(response.wsgi_request)]
...
class CameraViewTests(TestCase):
    ...
    def test_camera_add_post(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn("Camera added!", messages)
    ...
    def test_camera_edit_post(self):
        camera = baker.make(Camera, owner=self.user)
        response = self.client.post(
            reverse("camera-edit", args=(camera.id,)),
            data={
                "name": "A new name",
                "format": "135",
            },
    ...
    def test_camera_detail(self):
        camera = baker.make(Camera, owner=self.user)
        response = self.client.get(reverse("camera-detail", args=(camera.id,)))
        self.assertEqual(response.status_code, 200)
    ...
    def test_camera_or_back_detail_htmx(self):
        ...
        response1 = self.client.get(
            reverse("camera-detail", args=(camera.id,)), **headers1
        )
        ...
        response2 = self.client.get(
            reverse("camera-detail", args=(camera2.id,)), **headers2
        )
...
class FrameViewTests(TestCase):
    ...
    def test_frame_create(self):
        response = self.client.post(
            reverse("roll-frame-add", args=(self.roll.id,)),
            data={
                "number": "2",
                "date": self.today,
                "aperture": "1",
    ...
    def test_frame_create_and_add_another(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, expected_url=reverse("roll-frame-add", args=(self.roll.id,))
        )
        self.assertIn("Frame saved!", messages)
    ...
    def test_frame_create_and_add_another_with_inputs(self):
        ...
        baker.make(
            Frame, roll=self.roll, date=yesterday, aperture="abcd", shutter_speed="efgh"
        )
        response = self.client.get(
            reverse("roll-frame-add", args=(self.roll.id,)) + "?another"
        )
    ...
    def test_frame_create_and_add_another_with_presets(self):
        yesterday = datetime.date.today() - datetime.timedelta(days=1)
        baker.make(
            Frame, roll=self.roll, date=yesterday, aperture="2", shutter_speed="1/500"
        )
        response = self.client.get(
            reverse("roll-frame-add", args=(self.roll.id,)) + "?another"
        )
    ...
    def test_frame_create_error(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn("This roll already has frame #1.", messages)
    ...
    def test_frame_update_page_with_presets(self):
        baker.make(
            Frame, roll=self.roll, number="2", aperture="1.4", shutter_speed="1/500"
        )
    ...
    def test_frame_update_page_with_inputs(self):
        baker.make(
            Frame, roll=self.roll, number="2", aperture="abcd", shutter_speed="efgh"
        )
        response = self.client.get(reverse("roll-frame-edit", args=(self.roll.id, 2)))
    ...
    def test_frame_update(self):
        ...
        response = self.client.post(
            reverse("roll-frame-edit", args=(roll.id, frame_number)),
            data={
                "number": "2",
                "date": self.today,
                "aperture": "1.4",
    ...
    def test_frame_edit_error(self):
        ...
        response = self.client.post(
            reverse("roll-frame-edit", args=(roll.id, 2)),
            data={
                "number": "1",
                "date": self.today,
            },
    ...
    def test_frame_delete(self):
        ...
        response = self.client.post(
            reverse("roll-frame-delete", args=(roll.id, frame.number))
        )
        messages = [m.message for m in get_messages(response.wsgi_request)]
...
class StockViewTests(TestCase):
    ...
    def test_stocks_page(self):
        ...
        self.assertContains(
            response, f"{self.public_stock.manufacturer.name} {self.public_stock.name}"
        )
        self.assertContains(
            response,
            f"{self.personal_stock.manufacturer.name} {self.personal_stock.name}",
        )
    ...
    def test_stocks_page_with_unavailable_type(self):
        response = self.client.get(
            reverse("stocks-manufacturer", args=(self.public_stock.manufacturer.slug,))
            + "?type=bw"
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_stock_page(self):
        response = self.client.get(
            reverse(
                "stock",
                args=(self.public_stock.manufacturer.slug, self.public_stock.slug),
            )
        )
    ...
    def test_stocks_htmx_with_type(self):
        response = stocks(
            htmx_request(
                get_url=f"{self.stocks_url}?manufacturer=all&type=c41", user=self.user
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_stocks_htmx_with_manufacturer(self):
        response = stocks(
            htmx_request(
                get_url=f"{self.stocks_url}?manufacturer=kodak&type=c41", user=self.user
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Reset filters")
    ...
    def test_stocks_htmx_logged_out(self):
        ...
        response = stocks(
            htmx_request(get_url=f"{self.stocks_url}?manufacturer=kodak&type=c41")
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Reset filters")
    ...
    def test_personal_stock_logged_in(self):
        response = self.client.get(
            reverse(
                "stock",
                args=(self.personal_stock.manufacturer.slug, self.personal_stock.slug),
            )
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_personal_stock_404_logged_in(self):
        ...
        response = self.client.get(
            reverse("stock", args=(stock.manufacturer.slug, stock.slug))
        )
        self.assertEqual(response.status_code, 404)
...
class StockAddTests(TestCase):
    ...
    def test_stock_add_page_with_destination(self):
        response = self.client.get(reverse("stock-add") + "?destination=add-logbook")
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Add Film Stock")
    ...
    def test_adding_new_stock_and_new_manufacturer(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn(
            "New film stock “Fujifilm Provia 100” (in 35mm, 120) added!", messages
        )
    ...
    def test_adding_new_stock_from_inventory_and_another(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertIn(
            "New film stock “Kodak Tri-X 400” (in 35mm, 120) added!", messages
        )
