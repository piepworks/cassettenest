...
const checkAutocomplete = ($el) => {
  ...
  if (lastLine.match(olRegex)) {
    const nextNumber = Number(olNumberRegex.exec(lastLine)[0]) + 1;
    const numberLength = nextNumber.toString().length + 2;
    $el.value = `${beforeCursor}${nextNumber}. ${afterCursor}`;
    $el.setSelectionRange(
      cursorPosition + numberLength,
      cursorPosition + numberLength,
    );
  }
